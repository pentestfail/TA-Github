...
def collect_events(helper, ew):
    ...
    helper.log_info("input_type=github_api_repos_issues input={0:s} message='Collecting events.'".format(inputname))
    ...
    opt_checkpoint = "github_api_repos_issues-{0:s}".format(inputname)
    ...
    try:
        ...
        helper.log_debug("input_type=github_api_repos_issues input={0:s} message='Last successful checkpoint time.' last_status={1:s}".format(inputname,json.dumps(last_status)))
    except Exception as e:
        helper.log_error("input_type=github_api_repos_issues input={0:s} message='Unable to retrieve last execution checkpoint!'".format(inputname))
        raise e
    ...
    try:
        ...
        while has_results:
            ...
            helper.log_debug("input_type=github_api_repos_issues input={0:s} message='Requesting issue data from Github API.' url='{1:s}' parameters='{2:s}'".format(inputname,url,json.dumps(parameter)))

            # Return API response code
            r_status = response.status_code
            # Return API request status_code
            if r_status is not 200:
                helper.log_error("input_type=github_api_repos_issues input={0:s} message='API request unsuccessful.' status_code={1:d}".format    (inputname,r_status))
                response.raise_for_status()
            # Return API request as JSON
            obj = response.json()

            if obj is None:
                helper.log_info("input_type=github_api_repos_issues input={0:s} message='No records retrieved from Github API.'".format    (inputname))
                has_results = False
            ...
            helper.log_debug("input_type=github_api_repos_issues input={0:s} processed={1:d}".format(inputname,i))

            if has_results:
                helper.log_debug("input_type=github_api_repos_issues input={0:s} message='Getting next page.' link_next='{1:s}'".format(inputname,url))
                ...
                helper.log_debug("input_type=github_api_repos_issues input={0:s} message='No additional pages.'".format(inputname))
        ...
        helper.log_info("input_type=github_api_repos_issues input={0:s} message='Collection complete.' indexed={1:d}".format(inputname,i))
        helper.log_debug("input_type=github_api_repos_issues input={0:s} message='Storing checkpoint.' updated={1:s}".format(inputname,updated))

    except Exception as error:
        helper.log_error("input_type=github_api_repos_issues input={0:s} message='An unknown error occurred!'".format(inputname))
        raise error
